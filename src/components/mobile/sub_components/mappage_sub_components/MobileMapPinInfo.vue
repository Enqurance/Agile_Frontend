<template>
  <div>
    <van-dialog
      v-model:show="drawer"
      :z-index="1000"
      style="height: 80%; overflow-y: auto; padding: 3% 0"
      :scrollable="true"
        ...
        <el-card :body-style="{ padding: '10px' }">
          <template #header>
            <div class="card-header">
              ...
              <el-col :span="14" :offset="2" style="text-align: right">
                <van-button
                  v-if="
                    (info.visibility === 0 ||
                      this.$cookies.get('user_type') ===
          ...
          <!-- <p> {{ pin_tag }}</p> -->
          <p v-if="_get_pin_type(info.pin_type)">
            类型：{{ _get_pin_type(info.pin_type) }}
          </p>
          ...
          <template #header>
            <div class="card-header">
              ...
              <el-upload
                ...
                action="https://buaamapforum.cn:8443/photo/uploadPinPhoto"
          ...
          <el-carousel trigger="click" height="150">
            <el-carousel-item v-for="photoUrl in photos" :key="photoUrl">
              <img
                :src="photoUrl"
                class="photo"
                @contextmenu.prevent="deletePhoto(photoUrl)"
              />
            </el-carousel-item>
          </el-carousel>
        </el-card>
    ...
    <van-dialog
      v-model:show="show_feedback"
      :z-index="2000"
      style="background: #fcfcfc; padding: 3% 0"
      :show-cancel-button="true"
      @confirm="apply_feedback"
      title="反馈公共钉"
    >
    ...
    <van-dialog
      v-model:show="show_apply"
      :z-index="2000"
      style="background: #fcfcfc; padding: 3% 0"
      :show-cancel-button="true"
      @confirm="apply_public"
    >
          ...
          <van-field name="类别" label="类别">
            <template #input>
              ...
              >
                ...
                <van-radio style="margin-bottom: 4px" :name="7"
                  >生活服务</van-radio
                >
              </van-radio-group>
            </template>
...
export default {
  ...
  emits: ["close_drawer", "update_info"],
  setup() {
  ...
  methods: {
      ...
      that.$axios
        ...
        .then((res) => {
          ...
          that.$axios
            .get(
              "/examine/" +
                (res.data.data.pin.visibility === 0
                  ? "get_pin_state/"
                  : "get_public_pin_state/") +
                that.id,
              {
                headers: {
                  token: that.$cookies.get("user_token"),
    ...
    deletePhoto(photoUrl) {
      ...
      if (
        that.info.visibility === 1 &&
        that.$cookies.get("user_type") === "0"
      ) {
    ...
    apply_public() {
      ...
      that.$axios
        .get("/examine/apply_for_public/" + that.id, {
          headers: {
            token: that.$cookies.get("user_token"),
          },
        })
    ...
    apply_feedback() {
      ...
      that.$axios
        .post(
          "/examine/apply_for_feedback/" + that.id,
          {
